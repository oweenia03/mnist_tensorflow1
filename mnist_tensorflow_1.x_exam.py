...
# TensorFlow 1.x 호환을 위해 eager execution 비활성화
...
with tf.compat.v1.Session() as sess:
    ...
    for epoch in range(num_epochs):
        ...
        print("Epoch:", '%02d' % (epoch+1), "Loss:", "{:.4f}".format(avg_loss), "Accuracy:", "{:.4f}".format(acc * 100) + "%")

        # Re-initialize iterator for next epoch
        sess.run(iterator.initializer)

    print("Final Test Accuracy:", sess.run(accuracy, feed_dict={X: x_test, Y: y_test}) * 100, "%")
